...
public class Exam0140 {
  ...
  static class A4 extends A {
    // 필드 오버라이딩 - 필드 오버라이딩은 가급적 쓰지 않는다.
    // - 필드 오버라이딩은 메서드와 달리 변수의 타입이 달라도 된다.
    //
    String working;

    // this.필드명
    // - 현재 클래스에서 해당 필드(변수)를 찾는다. 없으면 상위 클래스로 따라 올라가면서 찾는다.
    // super.필드명
    ...
    void print() {
      ...
      System.out.printf("  => this.name(%s)\n", this.name);
      System.out.printf("  => this.tel(%s)\n", this.tel);
      System.out.printf("  => this.working(%s)\n", this.working); // 현재 클래스에서 필드 찾을 때는 this.
      System.out.printf("  => super.working(%b)\n", super.working); // Super 클래스에서 필드 찾을 때는 super.
    }
  }
