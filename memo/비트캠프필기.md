...
# 2022년 04월 01일
  - git 사용법
  - eomcs-docs user guide 참고
  - 책 git pro 2판(pdf도 있음)
## VSC(형상 관리 시스템 : Software Configuration System-SCM)
  - 로컬 버전 관리 시스템 : 공유불가(협업 작업 힘들다.)
  - 중앙집중식 버전관리 시스템 : 모든 변경내역을 보관하고있는 중앙 서버에 각각의 로컬 PC에서 생성된 프로젝트 소스들을 저장하여 공유한다.
    - 이러한 방식은 서버 저장소에 문제가 발생하면 과거의 모든 변경 내역에 문제가 발생할 위험이 있다.
  
  **snapshot (= commit)**
  - 분산 버전관리 시스템 : 로컬 PC에 있는 소스 저장소를 통째로 서버로 가져온다. ex) Git
    - 서버 저장소에 문제가 발생하더라도 로컬 저장소 중에 한 개를 선택하여 그대로 복원할 수 있다.  

- 소스를 꺼내고 저장하기
  - 중앙 집중식 : 1. 저장소에서 프로젝트로 소스를 checkout(저장소에서 소스를 꺼내는 것)
  2. 프로젝트에서 저장소로 commit(저장소에 소스를 저장하는 것)
  3. 저장소에서 프로젝트로 update(로컬 소스를 저장소에 있던 최신소스로 변경하는 것)

  - 분산 관리식 : 1. 로컬 저장소에 서버 저장소를 clone(서버저장소를 로컬에 복제하는 것)
  2. 로컬 저장소에서 프로젝트로 checkout(로컬 저장소에서 특정 버전의 sanpshot 소스를 꺼내는 것)
  3. 프로젝트에서 로컬저장소로 commit(변경한 소스를 로컬 저장소에 보관하는것)
  4. 로컬 저장소에서 서버 저장소로 push(로컬 저장소를 서버 저장소에 올리는 것) 
  5. 로컬 저장소에서 서버저장소를 pull(서버 저장소의 내용 중 로컬 저장소의 내용과 다른 것을 가져와서 합치는 것)

- .git 폴더가 로컬 저장소의 역할을 한다.
- 로컬 저장소와 파일 시스템

- git stauts : 현재 저장소의 상태(add, commit등)을 보여주는 명령어.
![](./images/2022-04-01-15-50-40.png)
- M 은 변경된 상태 (Modified)를 의미.
![](./images/2022-04-01-15-54-10.png)
- 초록색 M은 변경 된후 staged된 상태
![](./images/2022-04-01-15-56-41.png)
- A는 staged된 상태(아직 변경은 안된 상태)
![](./images/2022-04-01-15-59-23.png)
- AM은 staged된 후에 수정된 상태

- git config --system 설정정보 -> os의 전체 사용자에게 적용
- git config --global 설정정보 -> 특정 저장소에만 적용되는 설정

## git 저장소 만들기
- 1. 로컬 저장소를 만든다(자신의 pc에 폴더 만들기)
- 2. 서버 저장소를 만든다(git repo에 폴더만들기 - 로컬저장소와 이름 같게 하는게 좋음)
- 3. 로컬 저장소와 서버 저장소를 연결한다. - git remote add origin 연결하고자하는 repo주소

- git clone시 폴더이름 바꿔서 clone하는 방법
  - git clone https://github.com.eomjinyoung/mytest.git teacher_mytest
    - 뒤에 변경하고자하는 폴더 명을 명시해준다 
    - git clone [복제하고자 하는 repo주소] [변경하고자하는 폴더명]

- 변경된 파일을 변경 전으로 다시 복원하는 방법
  - git checkout 변경한 파일명
...
- git branch
  ...
  - git merge 데이터를 가져올 branch명 : 현재 작업 중인 branch에 다른 branch의 데이터를 합친다.
  - branch 삭제 : git branch -d 삭제할 branch명

- git에서 fork는 단순히 현재 상태를 복제하는 것으로 다른사람의 repo를 fork해온다고 해서 자동으로 업데이트 되지 않는다.